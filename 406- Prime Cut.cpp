...
int status[MAX];vector<int>prime;void seive(){ status[0]=1;    for(int i=4; i<MAX; i+=2) status[i]=1;    for(int i=3; i*i<=MAX; i+=2){        if(status[i]==0){            for(int j=3*i; j<MAX; j+=(2*i)){                status[j]=1;            }        }    }    for(int i=1; i<MAX; i++) if(!status[i]) prime.push_back(i);}
...
int main()
{
    ...
    while(cin>>n>>c){
        ...
        for(m=0; prime[m]<=n; m++);
        if(m%2==1){
            for(int i=max(0, m/2 -c+1); i<min(m, m/2 +c); i++) cout<<" "<<prime[i];
        }
        else{
            for(int i=max(0, m/2 -c); i<min(m, m/2 +c); i++) cout<<" "<<prime[i];
        }
        cout<<endl<<endl;
    }
    return 0;

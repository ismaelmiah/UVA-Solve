...
ll checkme(ll n){    ll a= sqrt(n), i=0, ch, kp;    while(a>0){        ch=i*i;        kp=a*a;        if(ch>n){            cout<<"No"<<endl;            break;        }        else if(kp + ch ==n ){            cout<<"Yes"<<endl;            break;        }        else if(kp + ch<n){            i++;        }        else{            a--;        }    }}
...
vector<int>prime;
void seive(){
    for(int i=4; i<10000005; i+=2) status[i]=1;
    for(int i=3; i*i<=10000005; i+=2){
        if(status[i]==0){
            for(int j=i*i; j<10000005; j+=(i+i)){
                status[j]=1;
            }
        }
    }
    for(int i=2; i<10000005; i++) if(!status[i]) prime.push_back(i);
}
...
int main()
{
    ll a, b, n, d, e, i, m, t, c=0, ck, dk, aa;
    while(scl(n) && n){
        c=0;
        n=n+1;
        while(n>=3){
